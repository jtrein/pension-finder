...
import { getAllPotsOverValue } from "../../services/getAllPotsOverValue";
...
export const getAllPotsOverValueController: RequestHandler = async (
  req,
  res
) => {
  ...
  const result = await getAllPotsOverValue(value);
  ...
  if (parsedResult.success === false) {
    ...
    throw new AppError({
      status: 500,
      message: `Something went wrong while getting all pots over value: ${value}`,
    });
  }
